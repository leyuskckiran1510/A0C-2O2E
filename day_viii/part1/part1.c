#if 0
gcc -o $0.out -Wall -Wextra  -pedantic $0 -lm && $0.out $@ #-fsanitize=address 
rm $0.out &>/dev/null
exit
#endif
...
uint32_t hash(char *key) {
  uint32_t hs = 0;
  hs = (hs << 8) |
       key[0]; // 0<<8 does nothing and hs = key[0] [key[0]==ff,for example]
  hs = (hs << 8) | key[1]; // shift's key[0] by 8bit to form
                           // 0xff00|key[1]==>0xffaa[key[1]==aa]
...
int main(int argc, char **argv) {
  ...
  uint32_t ans = hash("AAA");
  int count = 0;
  while (ans != dest) {
    // printf("%s = (%s, %s)\n",(char *)&ans,(char *)&hastable[ans][0],(char
    // *)&hastable[ans][1]);
    ans = hastable[ans][runs[count % runCount]];
    // printf("%x %s [%d] {%d}(%c)\n",ans,(char
    // *)&ans,count,runs[count%runCount],runs[count%runCount]?'R':'L');
    count++;
    // printf("%d\n",count);
  }
