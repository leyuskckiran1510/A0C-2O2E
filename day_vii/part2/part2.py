...
class Card:
    _HIGH_RULE = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
    ...
    def calc_worth(self):
        ...
        _worth = 0
        length = len(counts)
        if length == 1 and counts.get("J"):
            return 100
        most_cm = counts.most_common(2)
        if most_cm[0][0] == "J":
            counts[most_cm[1][0]] += most_cm[0][1]
        else:
            counts[most_cm[0][0]] += counts["J"]
        if "J" in counts:
            counts.pop("J")
            length = len(counts)
        if length == 1:
...
def main():
    ...
    with open(argv[1], "r") as fp:
        for _, i in enumerate(fp.readlines()):
